...
对每个问题，判断：
1. **可以自动回答** → 结合模板数据和第一次分析的建议答案，给出确定结论，标记 auto_resolved: true
2. **确实无法判断** → 仅当建议答案也不成立或涉及用户业务习惯时，才标记 auto_resolved: false

## 决策原则
- **优先采纳第一次分析的建议答案（suggested_answer）**。建议答案是第一次分析时 LLM 基于完整上下文给出的，通常正确
- 只有当前后数据矛盾、或涉及的的确是用户才能决定的业务规则时，才保留问题
- 不要因为"可能还有别的解释"就保留——如果只有一种合理解释，就确认它

## 常见可自动消化的情况
- 两个单元格的值相同 → 同一数据源，确认
- 模板已经标注了单位/标准号/计算公式 → 直接读出，确认
- 合并单元格范围暗示字段类型 → 确认推断
- 空白行是预留数据行 → 确认
- 签章区格式固定 → 确认
- 编号规则能从已有编号反推 → 确认规则
...
def _auto_review_uncertainties(
    ...
    questions_text = json.dumps([
        {"question_id": u.get("id", f"q{i}"), "question": u.get("question", ""),
         "suggested_answer": u.get("suggested_answer", ""),
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": (
                ...
                f"## 需要审核的问题\n{questions_text}\n\n"
                f"请逐个审核。优先采纳第一次分析的建议答案(suggested_answer)，只要合理就确认。只有确实无法从数据推断的才标记为未解决。"
            )},
        ],
        max_tokens=3000,
        temperature=0.1,
